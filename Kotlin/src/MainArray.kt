...
fun main() {
    ...
    for (arquivo in arquivos) {
        ...
        for ((nomePosicao, _) in posicoes) {
            medirTempoOperacao(carga, dados, n, "insertion", nomePosicao, "one_element") { lista, index ->
        ...
        for ((nomePosicao, _) in posicoes) {
            medirTempoOperacao(carga, dados, n, "insertion", nomePosicao, "n_elements") { lista, index ->
        ...
        // Operação 3: Get
        for ((nomePosicao, _) in posicoes) {
            medirTempoOperacao(carga, dados, n, "get", nomePosicao, "") { lista, index ->
        ...
        for ((nomePosicao, _) in posicoes) {
            medirTempoOperacao(carga, dados, n, "remove", nomePosicao, "one_element") { lista, index ->
                lista.remove(index)
            }
        }
        ...
        for ((nomePosicao, _) in posicoes) {
            medirTempoOperacao(carga, dados, n, "remove", nomePosicao, "n_elements") { lista, index ->
...
fun medirTempoOperacao(
    ...
    val arquivoSaida = File("Kotlin/out/ArrayList", nomeArquivo)
