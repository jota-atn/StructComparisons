...
auto test_fill_avltree(AVLTree& avltree, int n) {

    auto inicio = high_resolution_clock::now();
    
    avltree.fill_tree(to_string(n));
...
void test_dataset_fill_avltree(AVLTree& avltree, int num_execucoes) {
    ...
    string saida = "../out/avltree/fill_avltree.txt";
    ...
    for (int valor : values) {
        ...
        for (int i = 0; i < num_execucoes; i++) {
            tempo_total += test_fill_avltree(avltree, num_elementos);
        }
        ...
        cout << "Tempo medio de preenchimento de um AVLTree para um Data Set de tamanho " << to_string(valor)
