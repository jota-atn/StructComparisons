...
auto test_get_inicio(LinkedList& linked_list) {

    auto inicio = high_resolution_clock::now();

    int fim_val = linked_list.get_first();
...
void test_dataset_get_inicio(LinkedList& linked_list) {

    vector<int> values = {1000, 10000, 100000, 1000000, 10000000};

    string saida = "../out/linkedlist/get_inicio.txt";
    ...
    for (int valor : values) {
        ...
        for (int i = 0; i < num_execucoes; i++) {
            tempo_total += test_get_inicio(linked_list);
        }
        ...
        cout << "Tempo medio de get inicio para um Data Set de tamanho " << to_string(valor)
