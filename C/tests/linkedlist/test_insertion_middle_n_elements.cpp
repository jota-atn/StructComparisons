...
auto test_insert_middle(LinkedList& linked_list, int n) {
    
    auto inicio = high_resolution_clock::now();
    
    for (int i = 0; i < n; i++) {
        linked_list.add_middle(i);
    }
...
void test_dataset_insertion_middle_n_elements(LinkedList& linked_list, int num_execucoes) {
    ...
    string saida = "../out/linkedlist/insertion_mid_n_elements.txt";
    ...
    for (int valor : values) {
        ...
        for (int i = 0; i < num_execucoes; i++) {
            tempo_total += test_insert_middle(linked_list, num_elementos);
        }
        ...
        cout << "Tempo medio de insercao na Linked List sempre no meio de " << num_elementos
