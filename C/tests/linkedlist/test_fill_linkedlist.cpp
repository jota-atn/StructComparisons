...
auto test_fill_linkedlist(LinkedList& linked_list, int n) {

    auto inicio = high_resolution_clock::now();
    
    linked_list.fill_list(to_string(n));
...
void test_dataset_fill_linkedlist(LinkedList& linked_list, int num_execucoes) {
    ...
    string saida = "../out/linkedlist/fill_linkedlist.txt";
    ...
    for (int valor : values) {
        ...
        for (int i = 0; i < num_execucoes; ++i) {
            tempo_total += test_fill_linkedlist(linked_list, num_elementos);
        }
        ...
        cout << "Tempo medio de preenchimento de um linkedlist para um Data Set de tamanho " << to_string(valor)
