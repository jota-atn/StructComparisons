...
auto test_insert_middle_one_element(LinkedList& linked_list, int n) {

    auto inicio = high_resolution_clock::now();
    
    linked_list.add_middle(n);
...
void test_dataset_insertion_middle_one_element(LinkedList& linked_list, int num_execucoes) {
    ...
    string saida = "../out/linkedlist/insertion_mid_one_element.txt";
    ...
    for (int valor : values) {
        ...
        for (int i = 0; i < num_execucoes; ++i) {
            tempo_total += test_insert_middle_one_element(linked_list, elemento);
        }
        ...
        cout << "Tempo medio de insercao na Linked List sempre no meio de um unico elemento, " << elemento
