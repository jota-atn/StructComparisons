...
// testRemoveFirstOneElement mede o tempo para remover um elemento do início da lista
func testRemoveFirstOneElement(lista *linkedlist.LinkedList) int64 {
	inicio := time.Now()

	_, _ = lista.Remover(0) // Remove o primeiro elemento (índice 0)
...
// testDatasetRemoveFirstOneElement testa a remoção de um elemento do início para diferentes tamanhos de lista
func testDatasetRemoveFirstOneElement(lista *linkedlist.LinkedList) {
	...
	saida := "./out/linkedlist/remove_first_one_element.txt"
	...
	for _, valor := range valores {
		...
		for i := 0; i < numExecucoes; i++ {
			listaCopia := *lista
			tempoTotal += testRemoveFirstOneElement(&listaCopia)
		}
		...
		fmt.Printf("Tempo medio de remoção de 1 elemento do início para um Data Set de tamanho %d apos %d execucoes: %.4f nanossegundos\n",
...
func main() {
	...
	testDatasetRemoveFirstOneElement(lista)
}