...
// testInsertNElements mede o tempo para inserir n elementos na árvore AVL
func testInsertNElements(arvore *avltree.AVLTree, n int) int64 {
	inicio := time.Now()
...
// testDatasetInsertionNElements testa a inserção de n elementos para diferentes tamanhos de árvore
func testDatasetInsertionNElements(arvore *avltree.AVLTree) {
	...
	saida := "./out/avltree/insertion_n_elements.txt"
	...
	for _, valor := range valores {
		...
		for i := 0; i < numExecucoes; i++ {
			...
			tempoTotal += testInsertNElements(arvoreCopia, numElementos)
		}
		...
		fmt.Printf("Tempo medio de inserção de %d elementos para uma Árvore AVL de tamanho %d após %d execuções: %.4f nanossegundos\n",
...
func main() {
	...
	testDatasetInsertionNElements(arvore)
}
