...
// testFindSuccessor mede o tempo para encontrar o sucessor de um valor na árvore AVL
func testFindSuccessor(arvore *avltree.AVLTree, valor int) int64 {
	inicio := time.Now()

	_, _ = arvore.Sucessor(valor)
...
// testDatasetFindSuccessor testa a busca de sucessor para diferentes tamanhos de árvore
func testDatasetFindSuccessor(arvore *avltree.AVLTree) {
	valores := []int{1000, 10000, 100000, 250000, 500000, 600000, 750000, 1000000}

	saida := "./out/avltree/find_successor.txt"
	...
	for _, valor := range valores {
		...
		for i := 0; i < numExecucoes; i++ {
			// Escolhe um valor aleatório que não seja o máximo
			idx := rand.Intn(len(elementos) - 1) // -1 para evitar o último elemento
			valorTeste := elementos[idx]
			
			tempoTotal += testFindSuccessor(arvore, valorTeste)
		}
		...
		fmt.Printf("Tempo medio de busca de sucessor em uma Árvore AVL de tamanho %d após %d execuções: %.4f nanossegundos\n",
...
func main() {
	...
	testDatasetFindSuccessor(arvore)
}